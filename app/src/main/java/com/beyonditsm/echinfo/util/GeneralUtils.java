...
import com.beyonditsm.echinfo.activity.MainAct;
import com.beyonditsm.echinfo.entity.ResultData;
import com.beyonditsm.echinfo.entity.VersionDataEntity;
...
public class GeneralUtils {
    ...
    public void toVersion(final Context context, final int currentVersion,final int type){
        ...
        RequestManager.getCommManager().findVersion(currentVersion, "ANDROID", new CallBack() {
            @Override
            public void onSucess(String result) {
                ResultData<VersionDataEntity> rd = (ResultData<VersionDataEntity>) GsonUtils.json(result, VersionDataEntity.class);
                if (!rd.getData().isIsNeedUpdrage()) {
                    if (type == 0)
                        MyToastUtils.showShortToast(context, "当前已是最新版本！");

                } else {
                    String path = IEchinfoUrl.BASE_URL  + rd.getData().getVersion().getPackagePath();
...
//                        downLoad(context,path);
                ...
                },false).setNegativeButton("稍后再说", null).show();
//                    }
    ...
    private void downLoad(final Context context,String path) {
        ...
        HttpHandler handler = http.download(path, fileName, true, true,
                new RequestCallBack<File>() {
                    Intent intent = new Intent(MainAct.ISLOADING);
                    @Override
